...
List *merge(Node **left, Node **right, const Comparator compare) {
  ...
  while (*left != NULL && *right != NULL) {
    ...
    addLeft: {
      savTag = getTag(*left);
      merged = appendAndTag(merged, listPop(left), savTag);
      if (comp != Equal) continue;
    }

    addRight: {
      savTag = getTag(*right);
      merged = appendAndTag(merged, listPop(right), savTag);
    }
  }
...
List *sort(List *l, Comparator compare) {
  ...
  if (listSize <= 2) {
    ...
    while (i < listSize/2 && l->head != NULL) {
      ...
      left = appendAndTag(left, listPop(&l->head), savTag);
      ++i;
    }
    ...
    while (l->head != NULL) {
      savTag = getTag(l->head);
      right = appendAndTag(right, listPop(&l->head), savTag);
      ++i;
    }
