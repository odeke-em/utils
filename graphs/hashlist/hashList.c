...
inline int getHashListSize(HashList *hl) { return hl == NULL ? 0 : hl->size; }
...
inline unsigned int isEmptyRef(void **r) {
  return (r == NULL || *r == NULL) ? 1 : 0;
}
...
Element *elemPop(Element *e, void **popStore) {
  if (popStore != NULL && e != NULL) {
    *popStore = e->value;
    Element *nextTmp = e->next;
    free(e);
    e = nextTmp;
  }
...
Element *pop(HashList *hM, const hashValue hashCode) {
  ...
  if (getHashListSize(hM)) {
    unsigned int calcIndex = hashCode % getHashListSize(hM);
